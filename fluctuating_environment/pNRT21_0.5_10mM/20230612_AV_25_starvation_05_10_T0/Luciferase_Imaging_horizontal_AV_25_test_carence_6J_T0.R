...
setwd("/Users/vettoral/Documents/luciferase_imaging_horizontal/AV_25.1_pNRT21_carence/20230612_AV_25_starvation_05_10_T0/")
getwd()
...
testbox <- read.table("AV_25_20230612_test_carence_6J.txt",
                     header=TRUE, sep='\t') %>% 
  mutate(KNO3=factor(KNO3, levels = c("0.5mM","10mM")))
...
ggsave("compilation_DIFFERENCE_PLATE_AV_25.1_6j_T0.jpg")

######### Luciferase signal Raw integrated density #########


luc <- read.table("AV_25_20230612_test_carence_6J.txt",
                  header=TRUE, sep='\t') %>% 
  mutate(KNO3=factor(KNO3, levels = c("0.5mM","10mM")))
...
filter(luc, seedling!="background") %>% 
  ...
  geom_text(aes(label=CV , x=KNO3.x, y=220000 ),position=position_dodge(0.8)) +
  theme(legend.text=element_text(size=16),
        legend.title = element_text(size=18, face="bold"), 
        axis.text=element_text(size=16), 
        axis.title=element_text(size=18),
...
ggsave("Luciferase_signal_AV_25.1_6J_T0.jpg")
...
ggsave("Meannorm_data_length_norm_AV_25.1_6j_T0.jpg")
...
filter(luc, seedling!="background") %>% 
  ...
  geom_text(aes(label=CV , x=KNO3.x, y=10000 ),position=position_dodge(0.8)) +
  geom_text(aes(label=QCD , x=KNO3.x, y=11000 ),position=position_dodge(0.8)) +
  theme(legend.text=element_text(size=16),
        legend.title = element_text(size=18, face="bold"), 
        axis.text=element_text(size=16), 
        axis.title=element_text(size=18),
...
ggsave("RawIntDen_normBackg_normLen_AV_25.1_6J_T0.jpg")
...
filter(luc, seedling!="background") %>% 
  ...
  geom_boxplot(aes(color = genotype.x), width = 0.5, position = position_dodge(0.8),
               outlier.shape = NA) +
  geom_dotplot(aes(fill = genotype.x, color = genotype.x), binaxis='y', stackdir='center', 
               dotsize = 1, binwidth = 0.4, position = position_dodge(0.8))+
  scale_fill_manual(values = c("#00AFBB"), name="genotype", 
                    labels=c(expression(italic("pNRT2.1:LUC"))))+
  scale_color_manual(values = c("#00AFBB"))+
  geom_text(aes(label=n , x=KNO3.x, y=17 ),position=position_dodge(0.8)) +
    theme(legend.text=element_text(size=16),
        legend.title = element_text(size=18, face="bold"), 
        axis.text=element_text(size=16), 
        axis.title=element_text(size=18),
...
ggsave("length_AV_25.1_6J_T0.jpg")
