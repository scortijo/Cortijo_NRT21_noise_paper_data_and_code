...
setwd("/Users/vettoral/Documents/luciferase_imaging_horizontal/AV_25.1_pNRT21_carence/20230617_AV_25_starvation_05_10_T5/")
getwd()
...
testbox <- read.table("AV_25_test_carence_11J_T5.txt",
                     header=TRUE, sep='\t') %>% 
...
ggsave("compilation_DIFFERENCE_PLATE_AV_25.1_11j_T5.jpg")

######### Luciferase signal Raw integrated density #########


luc <- read.table("AV_25_test_carence_11J_T5.txt",
                  header=TRUE, sep='\t') %>% 
  mutate(KNO3=factor(KNO3, levels = c("0.5mM->10mM","0.5mM->0.5mM", "10mM->0.5mM", "10mM->10mM")))
...
ggsave("Luciferase_signal_AV_25.1_11J_T5.jpg")
...
ggsave("Meannorm_data_length_norm_AV_25.1_11j_T5.jpg")
...
filter(luc, seedling!="background") %>% 
  ...
  geom_text(aes(label=CV , x=KNO3.x, y=7000 ),position=position_dodge(0.8)) +
  geom_text(aes(label=QCD , x=KNO3.x, y=6000 ),position=position_dodge(0.8)) +
  theme(legend.text=element_text(size=16),
        legend.title = element_text(size=18, face="bold"), 
        axis.text=element_text(size=16), 
        axis.title=element_text(size=18),
...
ggsave("RawIntDen_normBackg_normLen_AV_25.1_10J_T4.jpg")
...
ggsave("length_AV_25.1_11J_T5.jpg")
