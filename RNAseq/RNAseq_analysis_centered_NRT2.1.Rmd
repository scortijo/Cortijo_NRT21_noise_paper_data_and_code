...
```{r}
Cluster1_long <- filter(CPM_SC18_all_WT_Zscore_BioVarsupp1, cluster_6_pearson==1) %>% 
  pivot_longer( cols = c(A1:A9), names_to = "seedling", values_to = "Zscore_normalised_expression") 
  

NRT2.1_long <- filter(CPM_SC18_all_WT_Zscore_BioVarsupp1, gene=="AT1G08090") %>% 
  pivot_longer( cols = c(A1:A9), names_to = "seedling", values_to = "Zscore_normalised_expression")  %>% 
  mutate(seedling=fct_reorder(seedling, Zscore_normalised_expression))
...
ggplot(NRT2.1_long, aes(x=seedling, y=Zscore_normalised_expression)) +
  ...
  geom_smooth(data=Cluster1_long, aes(x=seedling, y=Zscore_normalised_expression, group=cluster_6_pearson, color=cluster_6_pearson)) +
  ylab("Zscore normalised expression") +
