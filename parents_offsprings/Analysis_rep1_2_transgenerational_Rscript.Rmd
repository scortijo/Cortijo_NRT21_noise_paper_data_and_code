---
title: "rep1 - rep2 - transgenerational experiment"
author: "Sandra"
date: ""
...
Category and signal of parents 
...
Graph of selection of parents for rep1
...
ggplot(aes(x=genotype_Parent, y=RawIntDen_NormBack_NormLen_Parent, shape=type_Parent, col=Parent_name)) +
  geom_jitter(size=4, width = 0.03, height = 0) +
...
Graph of selection of parents for rep2
...
ggsave("Selection_parents_rep2_transGen.jpg",height = 5, width = 6)
...
Combining data of parents and offsprings

```{r, message=FALSE, warning=FALSE}

rep1_2_normwithParent <- left_join(rep1_2_norm, rep1_2_parents_norm, by=c("genotype"="Parent_name", "expt"="expt"))

```


Graphic of the distribution of the signal in the offspring depending on the signal of the parents 
...
filter(rep1_2_normwithParent, expt=="rep1") %>% 
mutate(type_Parent=factor(type_Parent, levels=c("low", "medium", "high"))) %>% 
  ggplot(aes(x=RawIntDen_NormBack_NormLen_Parent, y=RawIntDen_NormBack_NormLen, group= genotype, col=type_Parent)) +
  geom_boxplot( outlier.shape = NA, width=500 ) +
  ylab(expression(paste(italic("NRT2.1:LUC")," signal of offsprings"))) +
  xlab(expression(paste(italic("NRT2.1:LUC")," signal of parents"))) +
   scale_color_brewer(palette = "Dark2", name="Type of parent") +
  theme(legend.text=element_text(size=16),legend.title = element_text(size=16, face="bold"), 
        axis.text=element_text(size=16), axis.title=element_text(size=16),
        plot.title = element_text(size=20,face="bold"),
        panel.background = element_rect(fill = "white", colour = "grey50"),legend.key = element_rect(fill = NA, color = NA))
  
ggsave("Boxplot_transgenerational_rep1_parentsValues.jpg", height = 4, width = 5)



filter(rep1_2_normwithParent, expt=="rep1") %>% 
mutate(type_Parent=factor(type_Parent, levels=c("low", "medium", "high")),
        genotype=factor(genotype, levels=c("line2", "line3", "line6", "line9",
                                           "line11", "line12", "line13", "line14", "line17"))) %>% 
  ggplot(aes(x=RawIntDen_NormBack_NormLen_Parent, y=RawIntDen_NormBack_NormLen, group=genotype, col=genotype , shape=type_Parent)) +
  geom_point(size=2) +
  xlim(1000, 4200)+
  ylab(expression(paste(italic("NRT2.1:LUC")," signal of offsprings"))) +
  xlab(expression(paste(italic("NRT2.1:LUC")," signal of parents"))) +
...
ggsave("scatterplot_transgenerational_rep1_parentsValues.jpg", height = 5, width = 7)


# for rep2

filter(rep1_2_normwithParent, expt=="rep2") %>% 
mutate(type_Parent=factor(type_Parent, levels=c("low", "medium", "high"))) %>% 
  ggplot(aes(x=RawIntDen_NormBack_NormLen_Parent, y=RawIntDen_NormBack_NormLen, group= genotype, col=type_Parent)) +
  geom_boxplot( outlier.shape = NA, width=500 ) +
  ylab(expression(paste(italic("NRT2.1:LUC")," signal of offsprings"))) +
...
ggsave("Boxplot_transgenerational_rep2_parentsValues.jpg", height = 4, width = 5)



filter(rep1_2_normwithParent, expt=="rep2") %>% 
mutate(type_Parent=factor(type_Parent, levels=c("low", "medium", "high"))) %>% 
  ggplot(aes(x=RawIntDen_NormBack_NormLen_Parent, y=RawIntDen_NormBack_NormLen, group= genotype, shape=type_Parent, col=genotype)) +
  geom_point(size=2) +
  xlim(800, 3600)+
  ylab(expression(paste(italic("NRT2.1:LUC")," signal of offsprings"))) +
  xlab(expression(paste(italic("NRT2.1:LUC")," signal of parents"))) +
...
ggsave("Scatterplot_transgenerational_rep2_parentsValues.jpg", height = 5, width = 7)
